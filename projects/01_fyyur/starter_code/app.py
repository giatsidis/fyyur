...
db.create_all()
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:apostolis@localhost:5432/fyyur'
...
class Show(db.Model):
  ...
  venue_name = db.relationship('Venue', backref=db.backref('show'))
...
def create_venue_submission():
  try:
    ...
    # success message
    flash('Venue ' + form.name.data + ' was successfully added to the list!')
  except:
    # catches errors
    db.session.rollback()
    flash('An error had occured. Venue' + form.name.data + 'could not be added.')
  finally:
    db.session.close()
  return render_template('pages/home.html')
...
def edit_venue_submission(venue_id):
  try:
    ...
    vneue.seeking_talent = form.seeking_talent.data
    vneue.seeking_description = form.seeking_description.data
...
def create_artist_submission():
  ...
  try:
    ...
    db.session.commit()
    flash('Artist ' + form.name.data + ' was successfully added to the list!')
  except:
